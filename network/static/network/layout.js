...
function edit_post_submit(post_id) {
    ...
    .then(response => {
        if(response.success) {
            ...
            console.log('Ocorreu um erro ao editar a postagem.');
        }
    });
    remove_popup();
    return false;
...
function login_popup(action) {
    ...
    if(action === 'like') {
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Curta uma post para compartilhar apoio';
    }
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Comente para participar da conversa';
    }
    else if(action === 'save') {
        document.querySelector('.icon-div').innerHTML = `
